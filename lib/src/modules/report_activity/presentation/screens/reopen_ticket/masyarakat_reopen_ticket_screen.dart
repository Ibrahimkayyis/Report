...
class MasyarakatReopenTicketScreen extends StatefulWidget {
  ...
  const MasyarakatReopenTicketScreen({
    super.key,
    required this.ticketId,
  });

  @override
  State<MasyarakatReopenTicketScreen> createState() =>
      _MasyarakatReopenTicketScreenState();
}

class _MasyarakatReopenTicketScreenState
    extends State<MasyarakatReopenTicketScreen> {
  ...
  void _handleSubmit(BuildContext context) {
    ...
    showDialog(
      context: context, // Gunakan context yang dipassing (Child Context)
      builder: (dialogContext) => AppConfirmationDialog(
        ...
        onConfirm: () {
          ...
          // ✅ Sekarang aman memanggil read<ReopenTicketCubit>()
              ...
              if (state is TicketDetailLoading) {
                ...
                return Column(
                  children: [
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            _buildEditableTextField(
                              label: t.masyarakat.reopen_reason_label,
                              hint: t.masyarakat.reopen_reason_hint,
                    ...
                    // ✅ FIX: Pass 'context' (from BlocBuilder) to this method
  ...
  Widget _buildHeaderInfo({
    ...
    return Column(
      children: [
        Row(
          children: [
            Expanded(
              ...
              child: Text(
                "Pengirim",
                style: TextStyle(fontSize: 14.sp, fontWeight: FontWeight.bold, color: ColorName.textPrimary),
              ),
            ),
            Expanded(
              flex: 2,
              child: Row(
                children: [
                  CircleAvatar(
                    ...
                    child: profileUrl == null ? Icon(Icons.person, size: 24.sp, color: ColorName.primary) : null,
                  ),
                  SizedBox(width: 12.w),
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        Text(
                          fullName,
                          style: TextStyle(fontSize: 14.sp, fontWeight: FontWeight.w500, color: ColorName.textPrimary),
        ...
        SizedBox(height: 16.h),
        Row(children: [
          Expanded(flex: 1, child: Text("NIK", style: TextStyle(fontSize: 14.sp, fontWeight: FontWeight.bold, color: ColorName.textPrimary))),
          Expanded(flex: 2, child: Text(nik, style: TextStyle(fontSize: 14.sp, fontWeight: FontWeight.w500, color: ColorName.textPrimary))),
        ]),
        SizedBox(height: 16.h),
        Row(children: [
          ...
          Expanded(flex: 2, child: Container(
            ...
            decoration: BoxDecoration(color: Colors.grey.shade400, borderRadius: BorderRadius.circular(8.r)),
            child: Text(ticketCode, style: TextStyle(fontSize: 14.sp, fontWeight: FontWeight.w600, color: ColorName.black)),
          )),
        ]),
      ],
    );
  ...
  Widget _buildReadOnlyTextField({required String label, required String value, int minLines = 1}) {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      Container(
        ...
        decoration: BoxDecoration(color: Colors.grey.shade100, borderRadius: BorderRadius.circular(8.r), border: Border.all(color: Colors.grey.shade300)),
        child: Text(value, style: TextStyle(fontSize: 14.sp, color: Colors.grey.shade700, fontWeight: FontWeight.w500, height: 1.5)),
      ),
    ]);
  }
  ...
  Widget _buildEditableTextField({required String label, required String hint, required TextEditingController controller, String? errorText, int minLines = 1, required Function(String) onChanged}) {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      SizedBox(height: 8.h),
      TextField(
        controller: controller, onChanged: onChanged, maxLines: minLines, style: TextStyle(fontSize: 14.sp, color: ColorName.textPrimary),
        decoration: InputDecoration(
          hintText: hint, hintStyle: TextStyle(fontSize: 14.sp, color: Colors.grey.shade500), filled: true, fillColor: ColorName.white, contentPadding: EdgeInsets.all(14.w),
          border: OutlineInputBorder(borderRadius: BorderRadius.circular(8.r), borderSide: BorderSide(color: errorText != null ? Colors.red : Colors.grey.shade400)),
          enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(8.r), borderSide: BorderSide(color: errorText != null ? Colors.red : Colors.grey.shade400)),
          focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(8.r), borderSide: BorderSide(color: errorText != null ? Colors.red : ColorName.primary, width: 2)),
          errorText: errorText, errorStyle: TextStyle(fontSize: 12.sp),
        ),
      ),
    ]);
  }
